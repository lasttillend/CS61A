...
def has_cycle(link):
    ...
    """
    "*** YOUR CODE HERE ***"
    pos, k = -1, 0 # pos = -1 if we haven't found a cycle yet, otherwise positive
    seen = [] # keep track of which link objects we have already seen
    while link is not Link.empty and pos == -1:
        seen.append(link)
...
def has_cycle_constant(link):
    ...
    while fast is not Link.empty:
        if fast.rest == Link.empty:
            return False
        elif fast == slow or fast.rest == slow: # 如果有cycle，则fast迟早会追上slow
            return True
        else:
            slow, fast = slow.rest, fast.rest.rest # 每次fast都比slow多走一步
    return False
