...
def put_piece(board, max_rows, column, player):
    ...
    for row in range(max_rows):
        if get_piece(board, row, column) == '-':
            empty_row = row 
    if empty_row == -1: # the column is already full
        return -1, board
    else:
        new_row = replace_elem(board[empty_row], column, player)
        new_board = replace_elem(board, empty_row, new_row) # board也是一个list!!!可用new_row整行替换
        return empty_row, new_board
        
    # Method 2
    # for row in range(max_rows - 1, -1, -1): # 最底部的empty spot就是倒着数过来第一个
    #     if get_piece(board, row, column) == '-':
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert row < max_rows and row >= 0, 'Invalid row index!'
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert col < max_cols and col >= 0, 'Invalid col index!'
